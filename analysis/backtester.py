...
class StrategyBacktester:
    ...
    def _process_dollar_target(self, df, dollar_target, commission, slippage, position_size, contracts, contract_value):
        """
        Process backtest with fixed dollar profit target

        Parameters:
        df (pandas.DataFrame): DataFrame with signals
        dollar_target (float): Profit target in dollar amount
        commission (float): Commission percentage
        slippage (float): Slippage percentage
        position_size (float): Position size percentage
        contracts (int): Number of contracts per trade
        ...
        # Initialize results
        results = {
            'profit_target_dollar': dollar_target,
            'total_trades': 0,
            'winning_trades': 0,
            'losing_trades': 0,
            'winning_trades_dollar': 0,
        ...
        for entry_date, entry_row in entry_signals.iterrows():
            ...
            # Calculate risk and dollar-based profit target
            if signal_type == 'LONG':
                risk_amount = entry_price - stop_loss
                # Calculate the price movement needed to achieve dollar target
                price_movement = dollar_target / (contracts * contract_value)
                profit_target_price = entry_price + price_movement
            else:  # SHORT
                risk_amount = stop_loss - entry_price
                # Calculate the price movement needed to achieve dollar target
                price_movement = dollar_target / (contracts * contract_value)
